...
export default async function RequestQueuePage() {
  ...
  return (
    <main className="space-y-6">
      <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-900 pb-3 animate-fade-in-up">
        <div>
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-6 space-y-4 dark:border-zinc-900 dark:bg-zinc-950 shadow-sm animate-fade-in-up animation-delay-100">
        {pendingCount === 0 ? (
          <p className="text-sm text-zinc-500 dark:text-zinc-700 py-10 text-center">No pending requests in the queue.</p>
        ) : (
              ...
              <li key={r.id} className="flex flex-wrap items-center gap-4 px-4 py-3.5 sm:flex-nowrap">
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  <form action={approveRequest.bind(null, r.id)} className="flex-1 sm:flex-none">
                    <button className="w-full rounded-lg bg-emerald-600/10 dark:bg-emerald-600/20 px-3 py-1.5 text-xs font-medium text-emerald-700 dark:text-emerald-400 border border-emerald-500/10 hover:bg-emerald-600/20 dark:hover:bg-emerald-600/30 transition cursor-pointer">
                      Approve
                    </button>
                  </form>
                  <form action={rejectRequest.bind(null, r.id)} className="flex-1 sm:flex-none">
                    <button className="w-full rounded-lg bg-red-700/10 px-3 py-1.5 text-xs font-medium text-red-600 dark:text-red-400 border border-red-500/10 hover:bg-red-700/20 transition cursor-pointer">
                      Reject
                    </button>
                  </form>
                </div>
